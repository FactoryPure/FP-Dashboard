...
import "../brands/BrandScreen.css"
import Collection from "./Collection"

export default function CollectionScreen({ setScreen, search, setSearch }) {
    const { user, data, selected } = useSelector(state => state)
    const collections = data.collections
    const products = data.products 
    const navigate = useNavigate()
    const [visibleBrands, setVisibleBrands] = useState(50)
    const [filteredBrands, setFilteredBrands] = useState(collections)
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        setVisibleBrands(50)
        setFilteredBrands(collections.filter(c => c.title.toLowerCase().includes(search.toLowerCase())))
    }, [search])
    const Mag = () => {
        return (
    ...
    return (
        <div class="brands">
            ...
            <div className="products__main">
                {filteredBrands.slice(0, visibleBrands).map((b, index) => <Collection user={user} brand={b} products={products} key={b.title + "-" + index} search={search} setSearch={setSearch} />)}
